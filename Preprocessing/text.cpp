...
vector<Features> text::agregate(){ // A UPDATER A CHAQUE FOIS QU ON AJOUTE UN FEATURE //
    ...
    int sentenceC=0;
    for (int i=0;i<l;i++){
        res.push_back(Features(idAuthor,blockList.at(i).wordFrequency,blockList.at(i).wordCount,blockList.at(i).charFrequency,blockList.at(i).sentenceCount,blockList.at(i).unicWordsCount,blockList.at(i).differentWordsCount,blockList.at(i).subordinationsCount));
    }
    return res;
}
